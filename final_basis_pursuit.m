...
Pr_alloc=power_allocation_basis_purusit(Num_BS_Antennas,Num_BS_RFchains,BSAntennas_Index,G_BS,G_MS,K_BS,K_MS,Num_paths_est,Num_Qbits);
...
[F_HP(:,m)]=Basis_pursuit(F_UC(:,m),Num_BS_Antennas,Num_BS_RFchains,Num_Qbits);
end
for n=1:1:K_MS*Num_paths_est
[W_HP(:,n)]=Basis_pursuit(W_UC(:,n),Num_MS_Antennas,Num_MS_RFchains,Num_Qbits);
end
...
for m=1:1:K_BS*Num_paths_est
[F_HP_data]=Basis_pursuit(F_opt_est,Num_BS_Antennas,Num_BS_RFchains,Num_Qbits);
end
for n=1:1:K_MS*Num_paths_est
[W_HP_data]=Basis_pursuit(W_opt_est,Num_MS_Antennas,Num_MS_RFchains,Num_Qbits);
end
...
hold on;
plot(SNR_dBa,Rate_EstimatedCSI,'b','LineWidth',1.5);

xlabel('SNR (dB)','FontSize',18);
ylabel('Spectral Efficiency (bps/ Hz)','FontSize',18);
