...
Num_paths=q1([i2]); % Number of channel paths
Num_paths
...
G_BS=q3([i2]); % Required resolution for BS AoD
G_MS=q3([i2]); % Required resolution for MS AoA


K_BS=q2([i2]);  % Number of Beamforming vectors per stage 
K_MS=q2([i2]);% Number of Measurement vectors per stage
...
ITER=10; % Number of independent realizations (to be averaged over)
...
% Plotting the rates
j=i2;
plot(SNR_dBa,Rate_PerfectCSI,'color',C{j},'marker','o');
text(0,10,'Ld')
hold on;
%plot(SNR_dBa,Rate_EstimatedCSI,'color',C{j},'marker','+');
text(0,15,'Ld')
...
clearvars -except q1 q2 q3 i2 C
end