...
function [F]=Basis_pursuit(Fopt,Num_Antennas,Num_RFchains,Num_Qbits)
...
for m=1:1:Num_RFchains
    % Part 1- Selecting the best RF beamforming vector
    ...
    Fdd=inv(Frf'*Frf);
     Fbb=Fdd*Frf'*Fopt;
    Fres=(Fopt-Frf*Fbb)/sqrt(trace((Fopt-Frf*Fbb)'*(Fopt-Frf*Fbb)));
end

% Precoding vectors normalization

Fbb=(Fbb/sqrt(trace((Frf*Fbb)'*(Frf*Fbb))));
