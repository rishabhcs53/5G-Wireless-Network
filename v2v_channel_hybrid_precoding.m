%V2V Communication changes in Line Number 64-86
clear;clc;
...
for l=1:1:Num_paths
    ...
    Hl= 0;
    for y=1:1:1
        Hl=Hl+(alpha(l,y)*exp((1j*(2*pi)*(cos(0))*(Carrier_Freq)*(v))/(3*10^8)));
    end
...
plot(SNR_dBa,Rate_PerfectCSI,'r','LineWidth',1);
hold on;
plot(SNR_dBa,Rate_EstimatedCSI,'b','LineWidth',1);
