...
% Channel parameters (angles of arrival and departure and path gains)
 alpha=(sqrt(1/2)*sqrt(1/Num_paths)*(randn(Num_paths,1)+1j*randn(Num_paths,1)));
...
for l=1:1:Num_paths
    ...
    Hl= 0;
    for y=1:1:1
        Hl=Hl+(alpha(l,y)*exp((1j*(2*pi)*(cos(v))*(Carrier_Freq)*(v))/(3*10^8)));
    end
Abh(:,l)=sqrt(1/Num_BS_Antennas)*exp(1j*BSAntennas_Index*0);
Amh(:,l)=sqrt(1/Num_MS_Antennas)*exp(1j*MSAntennas_Index*0);
Channel=Channel+sqrt(Num_BS_Antennas*Num_MS_Antennas)*Hl*Amh(:,l)*Abh(:,l)';
end
...
[F_HP(:,m)]=HybridPrecoding2(F_UC(:,m),Num_BS_Antennas,Num_BS_RFchains,Num_Qbits);
end
for n=1:1:K_MS*Num_paths_est
[W_HP(:,n)]=HybridPrecoding2(W_UC(:,n),Num_MS_Antennas,Num_MS_RFchains,Num_Qbits);
end
...
for l=1:1:Num_paths_est
    Hl_est=0;
    for y=1:1:1
        Hl_est=Hl_est+(alpha_est(l,y)*exp((1j*(2*pi)*(cos(0))*(Carrier_Freq)*(v))/(3*10^8)));
    end
...
Channel_est=Channel_est+Hl_est*Amh_est(:,l)*Abh_est(:,l)';    
end
...
for m=1:1:K_BS*Num_paths_est
[F_HP_data]=HybridPrecoding2(F_opt_est,Num_BS_Antennas,Num_BS_RFchains,Num_Qbits);
end
for n=1:1:K_MS*Num_paths_est
[W_HP_data]=HybridPrecoding2(W_opt_est,Num_MS_Antennas,Num_MS_RFchains,Num_Qbits);
end
...
j=i2;
plot(SNR_dBa,Rate_PerfectCSI,'color',C{j},'marker',p{j});
hold on;
%plot(SNR_dBa,Rate_EstimatedCSI,'color',C{j},'marker',p{j});
title('Hybrid Precoding for V2V Channel at MM wave Considering relative velocity')
legend('Unconstrained Precoding - Perfect CSI')
xlabel('SNR (dB)','FontSize',12);
ylabel('Spectral Efficiency (bps/ Hz)','FontSize',12);

hold on;
hold on;
clearvars -except t1 i2 C p
end
